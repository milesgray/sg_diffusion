...
class ImageLogger(Callback):
    ...
    def _on_validation_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx, *args):
...
if __name__ == "__main__":
    ...
    try:
        ...
        # model
        add_modules = ["/content/CLIP", "/content/taming-transformers"]
        for m in add_modules:
            if m not in sys.path:
                sys.path.append(m)
        ...
        default_callbacks_cfg = {
            "setup_callback": {
                "target": "textual_inversion.train.SetupCallback",
                "params": {
                    "resume": opt.resume,
                    "now": now,
                    "logdir": logdir,
            ...
            "learning_rate_logger": {
                "target": "textual_inversion.train.LearningRateMonitor",
                "params": {
            ...
            "cuda_callback": {
                "target": "textual_inversion.train.CUDACallback"
            },
        }
        if version.parse(pl.__version__) >= version.parse('1.4.0'):
